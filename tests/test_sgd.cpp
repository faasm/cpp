...
TEST_CASE("Test setting up dummy data", "[sgd]")
{
    ...
    // Check weights
    const MatrixXd actualWeights =
      readMatrixFromState(WEIGHTS_KEY, 1, params.nWeights, true);
    REQUIRE(actualWeights.rows() == 1);
    REQUIRE(actualWeights.cols() == params.nWeights);

    const MatrixXd actualOutputs =
      readMatrixFromState(OUTPUTS_KEY, params.nWeights, params.nTrain, true);
...
TEST_CASE("Test hinge loss updates", "[sgd]")
{
    ...
    const MatrixXd actualWeights =
      readMatrixFromState(WEIGHTS_KEY, 1, nWeights, true);
    REQUIRE(actualWeights.rows() == 1);
...
void checkAppendOnlyInState(const std::string& user,
                            const char* key,
                            long nDoubles,
                            const std::vector<double>& expected)
{
    ...
    const std::shared_ptr<faabric::state::StateKeyValue>& kv =
      faabric::state::getGlobalState().getKV(user, key);
...
TEST_CASE("Test writing errors to state", "[sgd]")
{
    ...
    writeHingeError(params, a, b);
    checkAppendOnlyInState(
      getEmulatorUser(), ERRORS_KEY, 2, { expected1, expected2 });
}
