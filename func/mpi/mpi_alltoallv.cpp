...
int main(int argc, char* argv[])
{
    ...
    // Sendcount and send displacmements
    std::vector<int> sendCount(worldSize);
    std::vector<int> sdisps(worldSize);
    for (int i = 0, int acc = 0; i < worldSize; i++) {
        sendCount.at(i) = i + 1;
        sdisps.at(i) = acc;
        acc += i + 1;
    }

    // Recvcount and recv displacements
    std::vector<int> recvCount(worldSize, rank + 1);
    std::vector<int> rdisps(worldSize);
    for (int i = 0, int acc = 0; i < worldSize; i++) {
        rdisps.at(i)
    }
