...
std::string getEmulatorUser() { return _emulatedCall.user(); }

void setEmulatorUser(const char *newUser) { _emulatedCall.set_user(newUser); }
...
void setEmulatorState(faabric::state::State *state) { _emulatedState = state; }
...
void __faasm_write_output(const unsigned char *output, long outputLen) {
  faabric::util::getLogger()->debug("E - write_output {} {}", output,
                                    outputLen);
  _emulatedCall.set_outputdata(output, outputLen);
}

long __faasm_read_state(const char *key, unsigned char *buffer,
                        long bufferLen) {
...
void __faasm_read_state_offset(const char *key, long totalLen, long offset,
                               unsigned char *buffer, long bufferLen) {
  faabric::util::getLogger()->debug("E - read_state_offset {} {} {} {}", key,
                                    totalLen, offset, bufferLen);
  auto kv = getKv(key, totalLen);
...
unsigned char *__faasm_read_state_offset_ptr(const char *key, long totalLen,
  ...
  faabric::util::getLogger()->debug("E - read_state_offset_ptr {} {} {} {}",
                                    key, totalLen, offset, len);
  auto kv = getKv(key, totalLen);
...
void __faasm_read_appended_state(const char *key, unsigned char *buffer,
                                 long bufferLen, long nElems) {
  faabric::util::getLogger()->debug("E - read_appended_state {} {} {}", key,
                                    bufferLen, nElems);
...
unsigned int __faasm_write_state_from_file(const char *key,
                                           const char *filePath) {
  faabric::util::getLogger()->debug("E - write_state_from_file - {} {}", key,
                                    filePath);
...
void __faasm_flag_state_offset_dirty(const char *key, long totalLen,
                                     long offset, long dataLen) {
  // Avoid excessive logging
...
long __faasm_read_input(unsigned char *buffer, long bufferLen) {
  ...
  std::copy(_emulatedCall.inputdata().begin(), _emulatedCall.inputdata().end(),
            buffer);
...
unsigned int __faasm_chain_ptr(int (*funcPtr)(), const unsigned char *buffer,
                               long bufferLen) {
  throw std::runtime_error("Chaining not supported in emulator");
}

unsigned int __faasm_chain_py(const char *name, const unsigned char *buffer,
                              long bufferLen) {
  throw std::runtime_error("Chaining not supported in emulator");
}
...
int __faasm_await_call_output(unsigned int messageId, unsigned char *buffer,
                              long bufferLen) {
  throw std::runtime_error("Chaining not supported in emulator");
}
