...
SparseMatrixSerialiser::SparseMatrixSerialiser(
    const SparseMatrix<double> &matIn)
    : mat(matIn) {
  if (!mat.isCompressed()) {
    throw std::runtime_error(
        "Sparse matrices must be compressed before serializing");
  }
  ...
  // Outer index = number of non-zero entries in the columns _before_ this one
  // (has length cols + 1)
  //
  ...
  //
  // Both the values and inner arrays will have one entry for each value in
  // the matrix Outer index and non-zero count will have one entry for each
  // column
  ...
  // The non-zero counts can be worked out with the outer indices by
  // subtracting the ith value from the (i+1)th
...
SparseMatrixSerialiser::readFromBytes(const SparseSizes &sizes,
                                      uint8_t *outerBytes, uint8_t *innerBytes,
                                      uint8_t *valuesBytes) {
  ...
  Map<const SparseMatrix<double>> mat(sizes.rows, sizes.cols, sizes.nNonZeros,
                                      outerPtr, innerPtr, valuePtr);
...
Map<const SparseMatrix<double>> readSparseMatrixFromState(const char *key,
  ...
  return SparseMatrixSerialiser::readFromBytes(sizes, outerBytes, innerBytes,
                                               valuesBytes);
}

/**
 *  Reads a subset of a sparse matrix from state. The start/ end columns are
 * *exclusive*
 */
Map<const SparseMatrix<double>>
readSparseMatrixColumnsFromState(const char *key, long colStart, long colEnd,
                                 bool lazyPullFullValue) {
  // This depends heavily on the Eigen sparse matrix representation which is
  // documented here:
  // https://eigen.tuxfamily.org/dox/group__TutorialSparse.html
  ...
  uint8_t *nonZeroBuffer = faasmReadStateOffsetPtr(
      keys.nonZeroKey, sizes.nonZeroLen, colOffset, colBytes);
  ...
  uint8_t *outerBuffer = faasmReadStateOffsetPtr(keys.outerKey, sizes.outerLen,
                                                 colOffset, outerBytes);
  int *outerIndices = reinterpret_cast<int *>(outerBuffer);
  int startIdx = outerIndices[0];

  // We need to get these indices RELATIVE to the first index to fit our newly
  // created matrix BUT: we may be reusing THIS SAME ARRAY of outer indices so
  // we must copy it first before modifying
  ...
  uint8_t *valueBytes = faasmReadStateOffsetPtr(keys.valueKey, sizes.valuesLen,
                                                offsetValueBytes, nValueBytes);

  uint8_t *innerBytes = faasmReadStateOffsetPtr(keys.innerKey, sizes.innerLen,
                                                offsetInnerBytes, nInnerBytes);

  auto valuePtr = reinterpret_cast<double *>(valueBytes);
  auto innerPtr = reinterpret_cast<int *>(innerBytes);

  // Use eigen to import from the buffers
  Map<const SparseMatrix<double>> mat(sizes.rows, nCols, nValues,
                                      outerIndicesRebased, innerPtr, valuePtr);
 ...
 */
Map<const MatrixXd> readMatrixColumnsFromState(const char *key, long totalCols,
  ...
  // Get byte range from first element of first column, to last element of
  // last column
  ...
  long totalLen =
      getChunkSizeUpToMatrixElement(lastRowIdx, totalCols, totalRows);
  ...
  uint8_t *buffer =
      faasmReadStateOffsetPtr(key, totalLen, startOffset, bufferLen);
 ...
 */
double calculateSquaredError(const MatrixXd &prediction,
...
double calculateHingeError(const MatrixXd &prediction, const MatrixXd &actual) {
  ...
  for (long r = 0; r < prediction.rows(); r++) {
    for (long c = 0; c < prediction.cols(); c++) {
      ...
      // Product will be negative if prediction and actual have different
      // sign
