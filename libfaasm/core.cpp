...
void faasmReadStateOffset(const char *key, long fullLen, long offset,
                          uint8_t *buffer, long bufferLen) {
  __faasm_read_state_offset(key, fullLen, offset, buffer, bufferLen);
}

uint8_t *faasmReadStateOffsetPtr(const char *key, long fullLen, long offset,
                                 long len) {
...
void faasmReadAppendedState(const char *key, uint8_t *buffer, long bufferLen,
                            long nElems) {
...
void faasmWriteStateOffset(const char *key, long totalLen, long offset,
                           const uint8_t *data, long dataLen) {
...
void faasmFlagStateOffsetDirty(const char *key, long totalLen, long offset,
                               long len) {
  __faasm_flag_state_offset_dirty(key, totalLen, offset, len);
}

void faasmPushState(const char *key) { __faasm_push_state(key); }

void faasmPushStatePartial(const char *key) { __faasm_push_state_partial(key); }
...
void faasmLockStateGlobal(const char *key) { __faasm_lock_state_global(key); }

void faasmUnlockStateGlobal(const char *key) {
  __faasm_unlock_state_global(key);
}

void faasmLockStateRead(const char *key) { __faasm_lock_state_read(key); }

void faasmUnlockStateRead(const char *key) { __faasm_unlock_state_read(key); }

void faasmLockStateWrite(const char *key) { __faasm_lock_state_write(key); }

void faasmUnlockStateWrite(const char *key) { __faasm_unlock_state_write(key); }
...
unsigned int faasmAwaitCallOutput(unsigned int messageId, uint8_t *buffer,
                                  long bufferLen) {
  return __faasm_await_call_output(messageId, buffer, bufferLen);
}

unsigned int faasmChainNamed(const char *name, const uint8_t *inputData,
                             long inputDataSize) {
  return __faasm_chain_name(name, inputData, inputDataSize);
}

unsigned int faasmChain(FaasmFuncPtr funcPtr, const uint8_t *inputData,
                        long inputDataSize) {
